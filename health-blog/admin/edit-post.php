...
$scheduled_at = !empty($_POST['scheduled_at']) ? "'{$_POST['scheduled_at']}'" : "NULL";
    ...
    $sql = "UPDATE posts SET title='$title', content='$content', location='$location', category='$category', image='$all_images',tags='$tags',scheduled_at=$scheduled_at WHERE id=$id";
...
<div class="container mt-5">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <textarea name="content" id="content" rows="6" class="form-control" required><?php echo htmlspecialchars($post['content']); ?></textarea>
        </div>
                <div class="mb-3">
            <label for="image" class="form-label">Post Image</label>
 <input type="file" name="image[]" id="image" class="form-control" multiple >
 <small>Current: <?php echo $post['image']; ?></small>
<input type="datetime-local" name="scheduled_at" value="<?= date('Y-m-d\TH:i', strtotime($post['scheduled_at'])) ?>" class="form-control">
...
<div class="mb-3">
            ...
            <select name="category" id="category" onchange="otherSelect()" class="form-control" required>
                ...
                <option value="Recipes" <?php if ($post['category'] == 'Recipes') echo 'selected'; ?>>Recipes</option>
            </select>
        </div>
<div class="mb-3" id="otherBox" style="visibility: hidden;">
            <label for="title" class="form-label">Location</label>
            <input type="location" name="location" id="title" class="form-control" value="<?php echo htmlspecialchars($post['location']); ?>">
        </div>
...
const emails = <?php echo json_encode($tags); ?>; 
...
emails.forEach(tagText => {
    const tag = createTag(tagText);
    emailInput.append(tag);
});
updateEmailsInput();
</script>
</body>
